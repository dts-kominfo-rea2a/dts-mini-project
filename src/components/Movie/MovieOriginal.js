...
function MovieOriginal(){
    const [dataMovieOriginal, setDataMovieOriginal] = useState([]);
    const [selected, setSelected] = useState(null);
    useEffect(() => {
        getData();
    }, []);
    const getData = () => {
        Api.get('movie/now_playing?api_key=8152b136a5dad36d2ca7844f884577ba&language=en-US&page=1').then(function (response) {
            // handle success
            setDataMovieOriginal(response.data.results)
    ...
    return (
        ...
        <p className="font-medium text-2xl px-8">Now Playing</p>
        <Carousel
          scrollSnap={true}
          cols={5}
          rows={1}
                ...
                >
                  <div className="relative cursor-pointer">
                    ...
                    <img
                      ...
                      className={
                        selected === data.id
                          ? "w-[285px] h-[575px] scale-95 aspect-2/3 object-cover transition-all duration-100"
                          : "w-[285px] h-[575px] aspect-2/3 object-cover transition-all duration-100"
                      }
                    />
                  </div>
                </Link>
